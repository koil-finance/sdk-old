...
async function runQueryBatchSwapWithSor() {
    ...
    const poolsFetched = await koil.swaps.fetchPools();
    if (!poolsFetched) {
    ...
    let queryResult = await koil.swaps.queryBatchSwapWithSor({
    ...
    queryResult = await koil.swaps.queryBatchSwapWithSor({
        tokensIn: [
            STABAL3PHANTOM.address,
            STABAL3PHANTOM.address,
            STABAL3PHANTOM.address,
